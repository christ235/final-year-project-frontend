...
interface BusDetails {
  ...
  price: number
  fareType?: string
  // Additional bus details for seat selection
  stops?: number
...
export default function PassengerDetailsPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (outbound) {
        ...
        console.log("✅ Loaded outbound bus:", outboundData)
        console.log("👥 Initialized passengers:", passengersArray)
      }
      
      if (returnBus) {
        const returnData = JSON.parse(returnBus)
        setSelectedReturn(returnData)
        console.log("✅ Loaded return bus:", returnData)
      }
  ...
  const validateAllPassengers = () => {
    ...
    passengers.forEach((passenger, index) => {
      ...
      requiredFields.forEach(field => {
        if (!passenger[field as keyof PassengerInfo]) {
          allErrors[`${index}-${field}`] = `${field.replace(/([A-Z])/g, ' $1').toLowerCase()} is required`
          isValid = false
        }
      })
      ...
      // Email validation
      if (passenger.email && !/\S+@\S+\.\S+/.test(passenger.email)) {
        allErrors[`${index}-email`] = 'Please enter a valid email address'
        isValid = false
      }
    })
  ...
  const handleContinue = async () => {
    if (!selectedOutbound) {
      console.error("❌ No selected bus found")
      return
    }
    ...
    if (!validateAllPassengers()) {
      ...
      setCurrentPassengerIndex(0) // Go to first passenger with errors
      return
    }
    ...
    try {
      // Create contact info from main passenger
      ...
      const contactInfo: ContactInfo = {
        ...
        address: {
          street: "",
          city: selectedOutbound.trip.from,
          country: mainPassenger?.nationality || "Chad"
        }
      }

      // Prepare booking data for seat selection
      const bookingData: BookingData = {
        tripType,
        selectedBus: selectedOutbound,
      ...
      // Save booking data to sessionStorage for seat selection
      sessionStorage.setItem('bookingData', JSON.stringify(bookingData))
      console.log("💾 Booking data saved for seat selection:", bookingData)

      // Also save passenger details separately (for backward compatibility)
      ...
      router.push(`/dashboard/customer/travel/book/seat-selection?type=${tripType}`)
      
    } catch (error) {
      console.error("❌ Error preparing booking data:", error)
      setErrors({ submit: "Failed to save passenger details. Please try again." })
    } finally {
  ...
  const goBackToSearch = () => {
    if (tripType === "round-trip") {
      router.push("/dashboard/customer/travel/book/search-results-round-trip")
    } else {
    ...
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <button
            onClick={() => router.push("/dashboard/customer/travel/book")}
